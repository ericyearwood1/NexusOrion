...
namespace Meta.XR.BuildingBlocks.Editor
{
    ...
    public class SharedSpatialAnchorBuildingBlockEditor : BuildingBlockEditor
    {
        protected override void ShowAdditionals()
        {
            ...
            if (GUILayout.Button("Install Platform SDK"))
            {
                InstallPlatformSDK();
            }
        ...
        {
#if OVR_INTERNAL_CODE
            var packageId = "com.meta.xr.sdk.platform";
            var success = OVRProjectSetupUtils.InstallPackage(packageId);

            if (!success)
            {
                throw new InvalidOperationException($"Installation of package {packageId} failed.");
            }
            Debug.Log("Platform SDK installed successfully.");
#endif // OVR_INTERNAL_CODE
        }
    }
}
