...
class OVRExperimentalCheck : IPreprocessBuildWithReport
{
    ...
    public void OnPreprocessBuild(BuildReport report)
    {
        ...
        foreach (var device in devices)
        {
            ...
            var deviceIsExpEnabled = expSysPropSet == 1 || (IsUserDev(adbTool, device) && expSysPropSet != -1);
#else
...
#if OVR_INTERNAL_CODE
    static bool IsUserDev(OVRADBTool adbTool, string device)
    {
        if (TryGetSystemProperty(adbTool, device, "debug.oculus.failUserdevChecks", out var failUserdevChecks) &&
            failUserdevChecks != 0)
        {
            return false;
        }

        if (!TryGetSystemProperty(adbTool, device, "ro.build.fingerprint", "", out var fingerprint))
        {
            return false;
        }

        return fingerprint.Length - fingerprint.LastIndexOf("release-keys", StringComparison.Ordinal) != 12;
    }
