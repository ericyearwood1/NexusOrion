...
public class OVRSceneManager : MonoBehaviour
{
    ...
    [SerializeField]
    [Tooltip("When enabled, uses the experimental Anchor Discovery API. In the future, this option will be removed and the new anchor API will always be used.")]
    internal bool _useExperimentalAnchorApi;

    private static bool s_useExperimentalAnchorApi;
#endif // OVR_INTERNAL_CODE || OVR_PARTNER_CODE // XR_META_spatial_entity_discovery

    #region Events

#if OVR_PARTNER_CODE || OVR_INTERNAL_CODE // XR_META_return_to_room
    /// <summary>
    /// Invoked when the user enters an <see cref="OVRSceneRoom"/>
    /// </summary>
    /// <remarks>
    /// This event provides one argument:
    /// - <see cref="OVRSceneRoom"/>: The room the user entered
    /// </remarks>
    public UnityEvent<OVRSceneRoom> RoomEntered = new();

    /// <summary>
    /// Invoked when the user exits an <see cref="OVRSceneRoom"/>
    /// </summary>
    /// <remarks>
    /// This event provides one argument:
    /// - <see cref="OVRSceneRoom"/>: The room the user existed
    /// </remarks>
    public UnityEvent<OVRSceneRoom> RoomExited = new();
#endif
    ...
    public static class Classification
    {
        ...
        /// <summary>
        /// Represents an <see cref="OVRSceneAnchor"/> that is classified as a chair.
        /// </summary>
...
#pragma warning restore CS0618 // Type or member is obsolete
...
#if OVR_PARTNER_CODE || OVR_INTERNAL_CODE // XR_META_return_to_room
    /// <summary>
    /// Attempts to keep the user in a room
    /// </summary>
    /// <remarks>
    /// When enabled, the system will attempt to keep the user rooms spawned by the <see cref="OVRSceneManager"/> by
    /// displaying a system prompt if the user moves out of the room. Changing this setting at runtime does not affect
    /// rooms that have already been spawned by the <see cref="OVRSceneManager"/>. To change this setting on a per-room
    /// basis, use <see cref="OVRSceneRoom.ShouldAttemptToKeepUserInRoom"/>.
    /// </remarks>
    public bool KeepUserInRooms
    {
        get => _keepUserInRooms;
        set => _keepUserInRooms = value;
    }

    [SerializeField]
    internal bool _keepUserInRooms;

    bool _returnToRoomsChanged;

    readonly HashSet<Guid> _returnToRooms = new();

    internal void RequestReturnToRoom(Guid uuid, bool value)
    {
        if ((value && _returnToRooms.Add(uuid)) || (!value && _returnToRooms.Remove(uuid)))
        {
            _returnToRoomsChanged = true;
        }
    }
#endif
...
#if OVR_PARTNER_CODE || OVR_INTERNAL_CODE // XR_META_spatial_entity_discovery
        s_useExperimentalAnchorApi = _useExperimentalAnchorApi;
#endif
    }
    ...
    void Start()
    {
        OVRTelemetry.Start(OVRTelemetryConstants.Scene.MarkerId.UseOVRSceneManager)
            ...
            .AddAnnotation(OVRTelemetryConstants.Scene.AnnotationType.ReturnToRooms,
                KeepUserInRooms ? "true" : "false")
#endif
            .Send();
    }
    ...
    {
#if OVR_PARTNER_CODE || OVR_INTERNAL_CODE // XR_META_spatial_entity_discovery
        if (s_useExperimentalAnchorApi)
        {
            Development.Log(nameof(OVRSceneManager), $"Fetching anchors of type {typeof(T).Name} using xrDiscoverSpacesMETA.");
            ...
            {
                Development.Log(nameof(OVRSceneManager), $"xrDiscoverSpacesMETA completed with result {result.Status}.");
                task.SetResult(result.Success);
            });
    ...
    static OVRTask<bool> FetchAnchorsAsync(IEnumerable<Guid> uuids, List<OVRAnchor> anchors)
    {
        ...
        if (s_useExperimentalAnchorApi)
        {
            Development.Log(nameof(OVRSceneManager), $"Fetching {uuids.ToNonAlloc().GetCount()} anchors by UUID using xrDiscoverSpacesMETA.");
...
#if OVR_PARTNER_CODE || OVR_INTERNAL_CODE // XR_META_return_to_room
            sceneRoom.ShouldAttemptToKeepUserInRoom = _keepUserInRooms;
#endif
            sceneRoom.LoadRoom();
        }
    }
...
#if OVR_PARTNER_CODE || OVR_INTERNAL_CODE // XR_META_return_to_room
        if (_returnToRoomsChanged)
        {
            _returnToRoomsChanged = false;

            var result = OVRAnchor.RequestReturnToRoom(_returnToRooms);
            Development.Log(nameof(OVRSceneManager),
                message: $"RequestReturnToRoom([{string.Join(", ", _returnToRooms.Select(uuid => uuid.ToString()))}]) => {result.Status}",
                gameObject);
        }
#endif
    }
...
#if OVR_PARTNER_CODE || OVR_INTERNAL_CODE // XR_META_return_to_room
        OVRAnchor.RoomEntered += OnRoomEntered;
        OVRAnchor.RoomExited += OnRoomExited;
#endif
    }

#if OVR_PARTNER_CODE || OVR_INTERNAL_CODE // XR_META_return_to_room
    void OnRoomEntered(Guid uuid)
    {
        if (OVRSceneAnchor.SceneAnchors.TryGetValue(uuid, out var anchor) &&
            anchor.TryGetComponent<OVRSceneRoom>(out var room))
        {
            RoomEntered?.Invoke(room);
        }
    }

    void OnRoomExited(Guid uuid)
    {
        if (OVRSceneAnchor.SceneAnchors.TryGetValue(uuid, out var anchor) &&
            anchor.TryGetComponent<OVRSceneRoom>(out var room))
        {
            RoomExited?.Invoke(room);
        }
    }
#endif
