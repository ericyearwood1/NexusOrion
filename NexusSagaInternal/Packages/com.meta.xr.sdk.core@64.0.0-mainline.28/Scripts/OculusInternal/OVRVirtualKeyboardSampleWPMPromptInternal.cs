...
public class OVRVirtualKeyboardSampleWPMPromptInternal : MonoBehaviour
{
    ...
    private void Start()
    {
        virtualKeyboard = GameObject.FindAnyObjectByType<OVRVirtualKeyboard>(FindObjectsInactive.Include);
        ...
        virtualKeyboard.EnterEvent.AddListener(OnEnter);
    }
    ...
    private void CycleWritingPrompt()
    {
        ...
        if (_activeWritingPrompt != null)
        {
            ...
            statsString = $"Stats: {StatToText(activeStat)}";
            if (_activeWritingPrompt.IsComplete)
            {
        ...
        if (recordStat.wordsPerMinute != 0)
        {
            statsString += $" Record: {StatToText(recordStat)}";
        }
        if (string.IsNullOrWhiteSpace(statsString))
        {
            statsString = CompletePromptRichText;
        }
        statsOutput.text = statsString;
