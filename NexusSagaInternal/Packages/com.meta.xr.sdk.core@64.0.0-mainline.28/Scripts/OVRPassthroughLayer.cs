...
public class OVRPassthroughLayer : MonoBehaviour
{
    ...
    public void SetColorMapHandsContrast(float offset, float multiplier, float target)
    {
        ...
        colorMapEditorType = ColorMapEditorType.Custom;
        _stylesHandler.SetColorMapHandsContrast(offset, multiplier, target);
        styleDirty = true;
    }
...
#if OVR_INTERNAL_CODE
        private HandContrastStyleHandler _handContrastStyleHandler;
...
#if OVR_INTERNAL_CODE
            _handContrastStyleHandler = new HandContrastStyleHandler(ref _colorMapDataHandle, _colorMapData);
#endif // OVR_INTERNAL_CODE
        }
        ...
        private IStyleHandler GetStyleHandler(ColorMapType type)
        {
            switch (type)
            {
                ...
                case ColorMapType.HandsContrast:
                    return _handContrastStyleHandler;
#endif //OVR_INTERNAL_CODE
                default:
...
#if OVR_INTERNAL_CODE
        internal void SetColorMapHandsContrast(float offset, float multiplier, float target)
        {
            SetStyleHandler(ColorMapType.HandsContrast);
            _handContrastStyleHandler.Update(offset, multiplier, target);
        }
#endif //OVR_INTERNAL_CODE
    }
...
#if OVR_INTERNAL_CODE
            Debug.Assert(style.TextureColorMapType == ColorMapType.None || _colorMapData != null,
                "Color map not allocated.");
            Debug.Assert(style.TextureColorMapType == ColorMapType.None || _colorMapDataHandle.IsAllocated,
                "Passthrough color map enabled but data isn't pinned.");
#endif //OVR_INTERNAL_CODE
...
#if OVR_INTERNAL_CODE
                Debug.Assert(!_colorMapDataHandle.IsAllocated,
...
#if OVR_INTERNAL_CODE
    private class HandContrastStyleHandler : BaseGeneratedStyleHandler
    {
        protected override uint MapSize => throw new NotImplementedException();

        public HandContrastStyleHandler(ref GCHandle colorMapDataHandler, byte[] colorMapData)
            : base(ref colorMapDataHandler, colorMapData)
        {
        }

        public void Update(float offset, float multiplier, float target)
        {
            AllocateColorMapData();

            byte[] bytes = BitConverter.GetBytes(offset);
            Buffer.BlockCopy(bytes, 0, _colorMapData, 0, 4);
            bytes = BitConverter.GetBytes(multiplier);
            Buffer.BlockCopy(bytes, 0, _colorMapData, 4, 4);
            bytes = BitConverter.GetBytes(target);
            Buffer.BlockCopy(bytes, 0, _colorMapData, 8, 4);
        }
    }
#endif //OVR_INTERNAL_CODE
