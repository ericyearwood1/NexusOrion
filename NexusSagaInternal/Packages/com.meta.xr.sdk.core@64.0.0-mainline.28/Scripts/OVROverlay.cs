...
public class OVROverlay : MonoBehaviour
{
    ...
    // A factor by which the color value of the overlay is multiplied during compositing.
    // This field ignored if `overrideBlendFactors` is false.
    public OVRPlugin.BlendFactor srcBlendFactor = OVRPlugin.BlendFactor.SrcAlpha;

    // A factor by which the previous color value is multiplied during compositing.
    // This field ignored if `overrideBlendFactors` is false.
...
#if OVR_INTERNAL_CODE
    protected OVRPlugin.LayerDesc recommendationDesc;
    private float _layerVisibility = 0;

    public float layerVisibility
    {
        get
        {
    ...
    public void UpdateLayerVisibility()
    {
        ...
        {
            Debug.Log("Layer Visibility is not supported for layers that override source or destination rects");
            return;
        }

        OVRPose layerPose = OVRPose.identity;
        Vector3 layerScale = Vector3.one;
        bool overlay = false;
        bool headLocked = false;

        ComputePoseAndScale(out layerPose, out layerScale, out overlay, out headLocked);
        bool success = OVRPlugin.GetLayerVisibility(recommendationDesc, layerPose.ToPosef(), layerScale.ToVector3f(),
            overrideTextureRectMatrix, ref textureRectMatrix, out float visibility);
        if (!success)
        {
            Debug.LogError("GetLayerVisibility Call failed");
            layerVisibility = 0;
        }
        else
        {
...
#if OVR_INTERNAL_CODE
            // if using blend factors, we can specify premultiplied or unpremultiplied alpha,
            // so we don't need to do any math in the shader
            if (overrideBlendFactors && srcBlendFactor == OVRPlugin.BlendFactor.SrcAlpha)
                premultiplyAlpha = false;

            if (overrideBlendFactors && srcBlendFactor == OVRPlugin.BlendFactor.One)
...
#if OVR_INTERNAL_CODE
            , overrideBlendFactors, srcBlendFactor, dstBlendFactor
#endif
        );
