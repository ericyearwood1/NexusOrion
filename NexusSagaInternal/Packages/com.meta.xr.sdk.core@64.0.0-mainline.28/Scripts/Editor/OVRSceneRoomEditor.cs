    ...
    public override void OnInspectorGUI()
    {
        ...
        if (_showRuntimeValues)
        {
            ...
            using (new EditorGUI.DisabledScope(disabled: true))
            {
                ...
                try
                {
                    // This could throw if we're in a bad state; don't want to pollute the Editor console
#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN || (UNITY_ANDROID && !UNITY_EDITOR)
                    labelText = ((OVRSceneRoom)target).Label;
#endif
                }
                ...
                labelText = string.IsNullOrEmpty(labelText) ? "(none)" : labelText;
                EditorGUILayout.TextField(new GUIContent(
                        text: "Label",
                        tooltip:
                        "The user-defined label associated with this room."),
                    labelText);
