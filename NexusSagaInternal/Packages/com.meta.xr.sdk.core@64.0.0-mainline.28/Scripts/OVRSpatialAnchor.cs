...
public partial class OVRSpatialAnchor : MonoBehaviour
{
    ...
    /// <summary>
    /// (Experimental) Saves a collection of <see cref="OVRSpatialAnchor"/>s.
    /// </summary>
    /// <remarks>
    /// :warning: This is an experimental API. It will eventually supersede the other `Save` methods in this class.
    ///
    /// This method is asynchronous; use the returned <see cref="OVRTask{TResult}"/> to be notified of completion.
    /// When saved, the <see cref="OVRSpatialAnchor"/> can be loaded by a different session. Use
    /// <see cref="LoadUnboundAnchorsAsync"/> to load some or all <see cref="OVRSpatialAnchor"/> at a future time.
    ///
    ...
    /// <summary>
    /// (Experimental) Saves the <see cref="OVRSpatialAnchor"/> to persistent storage.
    /// </summary>
    /// <remarks>
    /// :warning: This is an experimental API. It will eventually supersede the other `Save` methods in this class.
    ///
    ...
    /// <see cref="Uuid"/> to identify the same <see cref="OVRSpatialAnchor"/> at a future time.
    ///
    /// If you have a collection of anchors to save, it is more efficient to use <see cref="SaveAnchorsAsync"/>.
    /// </remarks>
    /// <returns>An awaitable <see cref="OVRTask{TResult}"/> representing the asynchronous save operation.</returns>
    ...
    /// <summary>
    /// (Experimental) Erase the <see cref="OVRSpatialAnchor"/> from persistent storage.
    /// </summary>
    /// <remarks>
    /// :warning: This is an experimental API. It will eventually supersede the other `Erase` methods in this class.
    ///
    /// This operation is asynchronous. Use the returned <see cref="OVRTask{TResult}"/> to track the result of the
    /// asynchronous operation.
    ///
    /// If you have a collection of anchors to save, it is more efficient to use <see cref="EraseAnchorsAsync"/>.
    /// </remarks>
    ...
    /// <summary>
    /// (Experimental) Erase a collection of <see cref="OVRSpatialAnchor"/>s from persistent storage.
    /// </summary>
    /// <remarks>
    /// :warning: This is an experimental API. It will eventually supersede the other `Erase` methods in this class.
    ///
...
#endif // OVR_PARTNER_CODE || OVR_INTERNAL_CODE // XR_META_spatial_entity_persistence
...
#pragma warning restore
    ...
    public struct LoadOptions
    {
        ...
        /// </summary>
        /// <remarks>
        /// This is only required when querying by <see cref="LocalGroupUuid"/>.
        /// </remarks>
#else
        /// <summary>
        ...
        /// </summary>
        public Guid? LocalGroupUuid { get; set; }
...
#pragma warning restore CS0618
    ...
    {
#if OVR_PARTNER_CODE || OVR_INTERNAL_CODE
        if (options.Uuids == null && options.LocalGroupUuid == null)
        {
            throw new InvalidOperationException(
                $"{nameof(LoadOptions)}.{nameof(LoadOptions.Uuids)} or {nameof(LoadOptions)}.{nameof(LoadOptions.LocalGroupUuid)} must not be null.");
        }
#else
    ...
    struct LoadUnboundAnchorsTaskData
    {
        public List<OVRAnchor> PooledAnchorList;
        public List<UnboundAnchor> UserOwnedUnboundAnchors;
    }

    static Action<
        OVRResult<List<OVRAnchor>, OVRAnchor.FetchResult>,
        OVRTask<OVRResult<List<UnboundAnchor>, OVRAnchor.FetchResult>>> _onFetchAnchorsComplete = (result, task) =>
    {
        if (!task.TryGetInternalData<LoadUnboundAnchorsTaskData>(out var internalData))
        {
            Debug.LogError($"Task {task} does not have data associated with it. This is likely a bug.");
            task.SetResult(OVRResult.From((List<UnboundAnchor>)null, OVRAnchor.FetchResult.FailureDataIsInvalid));
            return;
        }

        try
        {
            var unboundAnchors = internalData.UserOwnedUnboundAnchors;
            ...
            task.SetResult(OVRResult.From(unboundAnchors, result.Status));
        }
        finally
        {
            OVRObjectPool.Return(internalData.PooledAnchorList);
        }
    };

    /// <summary>
    /// (Experimental) Loads unbound anchors with specified UUIDs.
    /// </summary>
    /// <remarks>
    /// :warning: This method is experimental. It will replace <see cref="LoadUnboundAnchorsAsync(LoadOptions)"/> in
    /// v64 of the SDK.
    ///
        ...
        Action<List<UnboundAnchor>, int> onIncrementalResultsAvailable = null)
    {
        ...
        var task = OVRTask.FromGuid<OVRResult<List<UnboundAnchor>, OVRAnchor.FetchResult>>(Guid.NewGuid());
        task.SetInternalData(new LoadUnboundAnchorsTaskData
        {
            PooledAnchorList = anchors,
            UserOwnedUnboundAnchors = unboundAnchors,
        });

        OVRAnchor.FetchAnchorsAsync(anchors, new OVRAnchor.FetchOptions { Uuids = uuids },
            onIncrementalResultsAvailable == null ? null : (incrementalResults, staringIndex) =>
            {
                ...
                {
                    onIncrementalResultsAvailable(unboundAnchors, unboundAnchorStartingIndex.Value);
                }
            })
            .ContinueWith(_onFetchAnchorsComplete, task);

        return task;
    }
#endif // OVR_PARTNER_CODE || OVR_INTERNAL_CODE // XR_META_spatial_entity_discovery
