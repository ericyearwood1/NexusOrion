    ...
    protected void UpdateSkeleton()
    {
        ...
        for (var i = 0; i < _bones.Count; ++i)
        {
            ...
            else if (IsHandSkeleton(_skeletonType))
            {
                ...
                if (OVRManager._handSkeletonVersion == OVRHandSkeletonVersion.V1)
                {
...
#if OVR_INTERNAL_CODE
            } else if( OVRManager._handSkeletonVersion == OVRHandSkeletonVersion.V2 )
                {
                    boneTransform.rotation = data.BoneRotations[i].FromQuatf();
                }
#endif // OVR_INTERNAL_CODE
            }
            else
