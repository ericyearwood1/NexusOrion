...
public static class CanvasInstancer {
    ...
    public static void PlaceUIElementRoot(GameObject element, MenuCommand menuCommand) {
        ...
        //We add an OCUnitsSettings component as the sibling of the Canvas component if we can find one and
        //there isn't one already there
        if (parent.GetComponentInParent<Canvas>() is Canvas foundCanvas && foundCanvas.GetComponent<OCUnitsSettings>() == null) {
            Undo.AddComponent<OCUnitsSettings>(foundCanvas.gameObject);
        }
    ...
    static bool IsValidCanvas(Canvas canvas) {
        if (canvas == null || !canvas.gameObject.activeInHierarchy || canvas.transform.parent != null)
            return false;
